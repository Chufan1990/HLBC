...
bool ReedsShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
  ...
  if (std::abs(all_possible_paths[optimal_path_index].x.back() -
    ...
    for (size_t i = 0;
      ...
      ADEBUG(
          "types are " << all_possible_paths[optimal_path_index].segs_types[i]);
    }
    ADEBUG("x, y, phi are: "
...
bool ReedsShepp::GenerateRSP(const std::shared_ptr<Node3d> start_node,
                             const std::shared_ptr<Node3d> end_node,
                             std::vector<ReedsSheppPath>* all_possible_paths) {
...
bool ReedsShepp::SCS(const double x, const double y, const double phi,
                     std::vector<ReedsSheppPath>* all_possible_paths) {
  RSPParam SLS_param;
...
bool ReedsShepp::CSC(const double x, const double y, const double phi,
                     std::vector<ReedsSheppPath>* all_possible_paths) {
  RSPParam LSL1_param;
...
bool ReedsShepp::CCC(const double x, const double y, const double phi,
                     std::vector<ReedsSheppPath>* all_possible_paths) {
  RSPParam LRL1_param;
...
bool ReedsShepp::CCCC(const double x, const double y, const double phi,
                      std::vector<ReedsSheppPath>* all_possible_paths) {
...
bool ReedsShepp::CCSCC(const double x, const double y, const double phi,
                       std::vector<ReedsSheppPath>* all_possible_paths) {
...
void ReedsShepp::LSL(const double x, const double y, const double phi,
                     RSPParam* param) {
...
void ReedsShepp::LSR(const double x, const double y, const double phi,
                     RSPParam* param) {
  std::pair<double, double> polar =
      common::math::Cartesian2Polar(x + std::sin(phi), y - 1.0 - std::cos(phi));
...
void ReedsShepp::LRL(const double x, const double y, const double phi,
                     RSPParam* param) {
  std::pair<double, double> polar =
      common::math::Cartesian2Polar(x - std::sin(phi), y - 1.0 + std::cos(phi));
...
void ReedsShepp::SLS(const double x, const double y, const double phi,
                     RSPParam* param) {
...
void ReedsShepp::LRLRn(const double x, const double y, const double phi,
                       RSPParam* param) {
...
void ReedsShepp::LRLRp(const double x, const double y, const double phi,
                       RSPParam* param) {
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
...
void ReedsShepp::LRSR(const double x, const double y, const double phi,
                      RSPParam* param) {
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
...
void ReedsShepp::LRSL(const double x, const double y, const double phi,
                      RSPParam* param) {
  double xi = x - std::sin(phi);
  double eta = y - 1.0 + std::cos(phi);
...
void ReedsShepp::LRSLR(const double x, const double y, const double phi,
                       RSPParam* param) {
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
  std::pair<double, double> polar = common::math::Cartesian2Polar(xi, eta);
...
bool ReedsShepp::SetRSP(const int size, const double* lengths,
                        const char* types,
                        std::vector<ReedsSheppPath>* all_possible_paths) {
  ReedsSheppPath path;
...
bool ReedsShepp::GenerateLocalConfigurations(
    ...
    const std::shared_ptr<Node3d> end_node, ReedsSheppPath* shortest_path) {
  double step_scaled =
...
void ReedsShepp::Interpolation(const int index, const double pd, const char m,
...
bool ReedsShepp::SetRSPPar(const int size, const double* lengths,
