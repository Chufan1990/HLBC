...
// bool FemPosDeviationOsqpInterface::Solve() {
//   if (ref_points_.empty()) {
//     AERROR("reference points empty. solver early terminated");
//     return false;
//   }

//   if (ref_points_.size() != bounds_around_refs_.size()) {
//     AERROR(
//         "reference points and bounds size not equal. solver early
//         terminated");
//     return false;
//   }

//   if (ref_points_.size() < 3) {
//     AERROR("reference points less than 3. solver early terminated");
//     return false;
//   }

//   if (ref_points_.size() > std::numeric_limits<int>::max()) {
//     AERROR("reference points too many. solver early teriminated");
//     return false;
//   }

//   // Calculate optimziation states definitions
//   num_of_points_ = static_cast<int>(ref_points_.size());
//   num_of_variables_ = num_of_points_ * 2;
//   num_of_constraints_ = num_of_variables_;
...
//   std::vector<c_int> A_indices;
//   std::vector<c_int> A_indptr;
//   std::vector<c_float> lowerbounds;
//   std::vector<c_float> upperbounds;
//   CalculateAffineConstraint(&A_data, &A_indices, &A_indptr, &lowerbounds,
//                             &upperbounds);
...
//   bool res = OptimizeWithOsqp(num_of_variables_, num_of_constraints_,
//   &P_data,
//                               &P_indices, &P_indptr, &A_data, &A_indices,
//                               &A_indptr, &lowerbounds, &upperbounds, &q,
//                               &primal_warm_start, data, &work, settings);
//   ADEBUG("5");
...
//   y_.resize(num_of_points_);
//   for (int i = 0; i < num_of_points_; i++) {
//     x_.at(i) = work->solution->x[i * 2];
//     y_.at(i) = work->solution->x[i * 2 + 1];
//   }
...
//   // Three quadratic penalties are involved:
//   // 1. Penalty x on distance between middle point and point by finite
//   element
//   // estimate;
//   // 2. Penalty y on path length;
//   // 3. Penalty z on difference between points and reference points

//   // General formulation of P matrix is as below(with 6 points as an
//   example):
//   // I is a two by two identity matrix, X, Y, Z represents x * I, y * I, z *
//   I
//   // 0 is a two by two zero matrix
//   // |X+Y+Z, -2X-Y,   X,       0,       0,       0    |
//   // |0,     5X+2Y+Z, -4X-Y,   X,       0,       0    |
...
//       weight_fem_pos_deviation_); columns[col_index].emplace_back(
//           col_index - 2,
//           -4.0 * weight_fem_pos_deviation_ - weight_path_length_);
//       columns[col_index].emplace_back(
//           col_index, 6.0 * weight_fem_pos_deviation_ +
...
// void FemPosDeviationOsqpInterface::CalculateOffset(std::vector<c_float>* q) {
//   q->resize(num_of_variables_, -2.0 * weight_ref_deviation_);
//   for (int i = 0; i < num_of_points_; i++) {
//     const auto& ref_point_xy = ref_points_[i];
//     q->at(2 * i) *= ref_point_xy.first;
//     q->at(2 * i + 1) *= ref_point_xy.second;
//   }
// }
...
//     A_data->push_back(1.0);
//     A_indices->push_back(i);
//     A_indptr->push_back(i);
//   }
//   A_indptr->push_back(num_of_variables_);
...
//   for (int i = 0; i < num_of_points_; i++) {
//     const auto& ref_point_xy = ref_points_[i];
//     upper_bounds->at(2 * i) = ref_point_xy.first + bounds_around_refs_[i];
//     upper_bounds->at(2 * i + 1) = ref_point_xy.second +
//     bounds_around_refs_[i]; lower_bounds->at(2 * i) = ref_point_xy.first -
//     bounds_around_refs_[i]; lower_bounds->at(2 * i + 1) = ref_point_xy.second
//     - bounds_around_refs_[i];
//   }
// }
...
//   for (int i = 0; i < num_of_points_; i++) {
//     const auto& ref_point_xy = ref_points_[i];
//     primal_warm_start->at(2 * i) = ref_point_xy.first;
//     primal_warm_start->at(2 * i + 1) = ref_point_xy.second;
//   }
// }
...
//     OSQPData* data, OSQPWorkspace** work, OSQPSettings* settings) {
//   CHECK_EQ(lower_bounds->size(), upper_bounds->size());
//   CHECK_EQ(lower_bounds->size(), num_affine_constraint);

//   data->n = kernel_dim;
//   data->m = num_affine_constraint;
//   data->P = csc_matrix(data->n, data->n, P_data->size(), CopyData(P_data),
//                        CopyData(P_indices), CopyData(P_indptr));

//   data->q = CopyData(q);
//   data->A = csc_matrix(data->m, data->n, A_data->size(), CopyData(A_data),
...
//   osqp_warm_start_x(*work, CopyData(primal_warm_start));
