...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  if (matrix_size != q_param_size || matrix_size != reverse_q_param_size) {
    ...
    error_msgs << "lateral controller error: matrix_q size: " << q_param_size
  ...
  /**
   *
   * TODO: Enable leadlag and MRAC here
   *
   */
  return Status::OK();
}
...
void LatController::LoadLatGainScheduler(
  ...
  for (const auto& scheduler : lat_err_gain_scheduler.scheduler()) {
    xy1.emplace_back(std::make_pair(scheduler.speed(), scheduler.ratio()));
  }

  for (const auto& scheduler : heading_err_gain_scheduler.scheduler()) {
    xy2.emplace_back(std::make_pair(scheduler.speed(), scheduler.ratio()));
  }
...
Status LatController::ComputeControlCommand(
    ...
    ControlCommand* cmd /* localization, chassis, trajectory, cmd */) {
  /**
   * @todo declare vehicle_state
   */
  ...
  //       // Within the low-high speed transition window, linerly interplolate
  //       the
  //       // augment control gain for "soft" control switch
  //       steer_angle_feedback_augment = common::math::lerp(
  //           steer_angle_feedback_augment, low_speed_bound_ -
  //           low_speed_window_, 0.0, low_speed_bound_,
  //           std::fabs(vehicle_state->linear_velocity()));
  //     }
  //   }
  // }
  ...
  //       steer_angle, steer_state, steer_limit, steer_diff_with_max_rate /
  //       ts_);
...
void LatController::UpdateState(SimpleLateralDebug* debug) {
  ...
  for (int i = 0; i < preview_window_; i++) {
    ...
    const auto matched_point = trajectory_analyzer_.QueryNearestPointByPoistion(
...
void LatController::ComputeLateralErrors(
  ...
  if (FLAGS_query_time_nearest_point_only) {
    target_point = trajectory_analyzer.QueryNearestPointByAbsoluteTime(
        ros::Time::now().toSec() + query_relative_time_);
  } else {
    if (FLAGS_use_navigation_mode &&
        !FLAGS_enable_navigation_mode_position_update) {
      target_point = trajectory_analyzer.QueryNearestPointByAbsoluteTime(
      ...
      target_point = trajectory_analyzer.QueryNearestPointByPoistion(x, y);
    }
  }
