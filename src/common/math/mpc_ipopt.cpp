...
bool MpcIpopt::Solve(
  ...
  for (int i = 0; i < mpc_ipopt->horizon_ - 1; i++) {
    auto& p = solution[i];
    ...
    p.set_steer(ret.x[mpc_ipopt->steer_index_ + i]);
    p.set_a(ret.x[mpc_ipopt->accel_index_ + i]);
  }

  auto p = solution.end() - 1;

  p->mutable_path_point()->set_x(
      ret.x[mpc_ipopt->x_index_ + mpc_ipopt->horizon_ - 1]);
  p->mutable_path_point()->set_y(
      ret.x[mpc_ipopt->y_index_ + mpc_ipopt->horizon_ - 1]);
  p->mutable_path_point()->set_theta(
      ret.x[mpc_ipopt->heading_index_ + mpc_ipopt->horizon_ - 1]);
  p->set_v(ret.x[mpc_ipopt->speed_index_ + mpc_ipopt->horizon_ - 1]);
  return ok;
}
