...
Status VehicleStateProvider::Update(
  ...
  if (!ConstructExceptLinearVelocity(localization)) {
    std::stringstream ss;
    ss << "Fail to update because ConstructExceptLinearVelocity error."
       << "localization:\n"
       << localization.DebugString();
    return Status(ErrorCode::LOCALIZATION_ERROR, ss.str());
  }
...
math::Vec2d VehicleStateProvider::ComputeCOMPosition(
  ...
  AERROR("\nFLAGS_state_transform_to_com_reverse: " << FLAGS_state_transform_to_com_reverse << "\nFLAGS_state_transform_to_com_drive: " << FLAGS_state_transform_to_com_drive << "\nvehicle_state_.gear(): " << vehicle_state_.gear());
